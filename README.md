...
- 🖥️ **One-Screen Layout**: Clock card, prayer schedule and credits are sized with viewport-height fluid `clamp()` values so the whole app fits a single screen (no vertical scrolling) from `1280x720` laptops down to `360x640` phones. Short, wide windows switch to a side-by-side clock + schedule layout, so the clock also works as a fullscreen backdrop.
...
10. **تخطيط بملء شاشة واحدة**: تُقاس البطاقة وجدول الصلاة بمسافات مرنة تعتمد على ارتفاع الشاشة (`clamp()` مع `svh`) فتظهر الساعة والمواقيت كلها في شاشة واحدة دون تمرير عمودي، من شاشات `1280x720` وحتى هواتف `360x640`. وفي النوافذ القصيرة والعريضة (مثل استخدام الساعة كخلفية) يتحوّل الجدول إلى عرض جانبي بجانب الساعة.
